...
function renderGraph() {
  ...
  nodes.select('.core').attr('fill',d=>sharedRoom?memberColor(d):null);nodes.select('.halo').attr('fill',d=>sharedRoom?memberColor(d):null);nodes.select('text').text(d=>d.label.length>18?d.label.slice(0,18)+'…':d.label);nodes.select('title').text(d=>sharedRoom?`${d.label} · ${memberFor(d)?.name || '알 수 없는 참여자'}`:d.label);nodes.classed('pending',d=>d.status==='pending').classed('failed',d=>d.status==='failed');
