...
let selected = null, busy = false, hovered = null, transform = d3.zoomIdentity, paused = false, activeSidebarTab = 'nodes', memberFilterId = null;
let controller;
...
function persist() { if(sharedRoom)return; write('ieum.graph', { nodes: state.nodes.map(({id,label,x,y,status,createdAt})=>({id,label,x,y,status,createdAt})), edges: state.edges.map(e=>({...e,source:idOf(e.source),target:idOf(e.target)})) }); }
...
function renderGraph() {
  ...
  const nodes=svg.select('.nodes').selectAll('g').data(state.nodes,d=>d.id).join(enter=>{const g=enter.append('g').attr('class','node');g.append('circle').attr('class','recent-ring').attr('r',25);g.append('circle').attr('class','halo').attr('r',0).transition().duration(reducedMotion?0:550).attr('r',19);g.append('circle').attr('class','core').attr('r',0).transition().duration(reducedMotion?0:550).ease(d3.easeBackOut.overshoot(2.5)).attr('r',6);if(!reducedMotion)g.append('circle').attr('class','entry-ring').attr('r',6).transition().duration(750).attr('r',34).style('opacity',0).remove();g.append('text').attr('text-anchor','middle').attr('y',26);g.append('title');return g;});
  ...
  const recentIds=recentNodeIds(state.nodes);nodes.style('--node-color',d=>sharedRoom?memberColor(d):null).select('text').text(d=>d.label.length>18?d.label.slice(0,18)+'…':d.label);nodes.select('title').text(d=>sharedRoom?`${d.label} · ${memberFor(d)?.name || '알 수 없는 참여자'}`:d.label);nodes.classed('pending',d=>d.status==='pending').classed('failed',d=>d.status==='failed').classed('recent',d=>recentIds.has(d.id));
...
function renderList(){ const query=$('#search').value.trim().toLowerCase(),member=sharedRoom?.members.find(item=>item.id===memberFilterId);if(!member)memberFilterId=null;$('#node-filter').hidden=!member;$('#node-filter-label').textContent=member?`${member.name} 님의 생각`:'';$('#node-list').innerHTML=state.nodes.filter(n=>(!memberFilterId||n.authorId===memberFilterId)&&n.label.toLowerCase().includes(query)).map(n=>`<button class="list-item ${selected===n.id?'is-selected':''}" data-id="${safe(n.id)}"><span class="list-dot" style="background:${safe(memberColor(n))}"></span><span>${safe(n.label)}</span><small>${related(n.id).size ? related(n.id).size-1:0}</small></button>`).join('')||'<p class="list-empty">생각이 없습니다.</p>';$('#node-list').querySelectorAll('button').forEach(b=>b.onclick=()=>{selectNode(state.nodes.find(n=>n.id===b.dataset.id));$('.sidebar').classList.remove('mobile-open');}); }
function renderMembers(){const tab=$('#members-tab');tab.hidden=!sharedRoom;if(!sharedRoom){memberFilterId=null;$('#members-panel').hidden=true;return;}$('#members-count').textContent=sharedRoom.members.length;const ranked=sharedRoom.members.map((member,index)=>{const authored=new Set(state.nodes.filter(node=>node.authorId===member.id).map(node=>node.id));const connections=state.edges.filter(edge=>authored.has(idOf(edge.source))||authored.has(idOf(edge.target))).length;return{...member,connections,index};}).sort((a,b)=>b.connections-a.connections||a.index-b.index);$('#member-list').innerHTML=ranked.map(member=>`<button class="member-item ${memberFilterId===member.id?'is-selected':''}" data-member-id="${safe(member.id)}" title="연결 ${member.connections}개"><span class="member-swatch" style="background:${safe(member.color)}"></span><span class="member-name">${safe(member.name)}</span>${member.role==='host'?'<span class="member-role">방장</span>':''}<small>${member.connections}</small></button>`).join('');$('#member-list').querySelectorAll('button').forEach(button=>button.onclick=()=>{const member=sharedRoom.members.find(item=>item.id===button.dataset.memberId);memberFilterId=member.id;selected=null;activeSidebarTab='nodes';updateSidebarTab();updateHighlight();renderDetail();renderList();$('.sidebar').classList.remove('mobile-open');});}
