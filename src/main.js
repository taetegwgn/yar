...
let selected = null, busy = false, hovered = null, transform = d3.zoomIdentity, paused = false;
let controller;
...
$('#app').innerHTML = `
  ...
  <div class="workspace"><aside class="sidebar"><div class="space-heading"><span>내 작업 공간</span><span id="room-role" class="mono">LOCAL</span></div><button class="workspace-link">${icon('orbit')}<span id="space-name">생각의 지도</span>${icon('chevron-down')}</button><div class="sidebar-title"><span>모든 생각 <b id="list-count"></b></span>${button('clear','trash-2','전체 삭제')}</div><div class="search">${icon('search')}<input id="search" aria-label="생각 검색" placeholder="생각 검색"></div><div id="node-list"></div><div class="sidebar-bottom"><span class="tiny-label">CONNECTION ENGINE</span><button id="engine"><span class="status-dot"></span><span id="engine-label"></span>${icon('arrow-up-right')}</button><span class="engine-note">Gemini 2.5 Flash</span></div></aside>
...
function renderGraph() {
  ...
  nodes.select('text').text(d=>d.label.length>18?d.label.slice(0,18)+'…':d.label);nodes.select('title').text(d=>d.label);nodes.classed('pending',d=>d.status==='pending').classed('failed',d=>d.status==='failed');
